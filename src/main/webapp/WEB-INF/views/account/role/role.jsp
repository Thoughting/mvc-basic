...
<div id="bd">
					...
					<td>&nbsp;
						<security:authorize ifAnyGranted="ROLE_浏览角色">
							<security:authorize ifNotGranted="ROLE_修改角色">
								<a href="role/${role.id}">查看</a>&nbsp;
							</security:authorize>
						</security:authorize>

						<security:authorize ifAnyGranted="ROLE_修改角色">
							<a href="${ctx}/account/role/${role.id}" id="editLink-${role.name}">修改</a>&nbsp;
							<a class="delete" href="${ctx}/account/role/${role.id}" id="deleteLink-${role.name}" data-remove="task_${role.id}">删除</a>
						</security:authorize>
					</td>
				</tr>
			</c:forEach>
	...
	<div>
		<security:authorize ifAnyGranted="ROLE_修改角色">
			<a href="${ctx}/account/role/new">增加新角色</a>
		</security:authorize>
	</div>
	</div>
	</div>
