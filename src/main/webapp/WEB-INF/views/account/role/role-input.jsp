...
<div id="bd">
	<div id="yui-main">
	<div class="yui-b">
	<h2><c:choose><c:when test="${role.id == null}">创建</c:when><c:otherwise>修改</c:otherwise></c:choose>角色</h2>
	<c:choose>
		<c:when test="${role.id == null}">
			<c:url var="actionUrl" value="account/role"/> 
			<c:url var="parameterMethod" value="post"/>
		</c:when>
		<c:otherwise>
		 	<c:url var="actionUrl" value="account/role/${role.id}"/> 
		 	<c:url var="parameterMethod" value="put"/>
		</c:otherwise>
	</c:choose>
	<form:form method="${parameterMethod}" commandName="role" action="${ctx}/${actionUrl}">
		<input type="hidden" name="id" value="${role.id}"/>
		<fieldset>  
                        <legend><fmt:message key="roleFields"/></legend>
                        <ul>
                        <li>  
                        ...
                        <li>
                        <form:label  for="authorityList" path="authorityList" cssErrorClass="error">  
                                <fmt:message key="authorityName"/>  
                            </form:label> 
                        <form:checkboxes itemValue="id" itemLabel="name" path="authorityList" items="${allAuthorityList}" delimiter="<br/>" onclick="yourOnClickMethodIfYouNeed(this);"/>
		...
		<table class="noborder">
			<tr>
				<td>角色名:</td>
				<td><input type="text" id="name" name="name" size="40" value="${role.name}" class="required"/></td>
			</tr>
			<tr>
				<td>授权:</td>
				<td>
